package def
{
	...
	public class NinjaAssetDef
	{
		...
		public static function getAsset(type:String,id:uint,playerBoo:Boolean=false,faceid:int=1):String
		{
			...
			{
				var ninjaCfgInfo:NinjaInfoCFG = NinjaInfoConfig.getNinjaCfgInfo(id);
				if(ninjaCfgInfo)
				{
					id = ninjaCfgInfo.artisticId;
				}
			}
			...
			if (playerBoo)
			{
				switch(type)
				{
					...
					case HEAD_BIG:
						...
						break;
					case HEAD_SMALL:
						if(id==BagUtils.PLAYER_NINJA_ID){//如果是主角碎片就要映射其他的样子				
							id = BagUtils.getPlayerFragmentResId();
						}
