package com.tencent.morefun.naruto.plugin.newplot.utils
{
	...
	public class AStarPathFinder
	{
		...
		public function AStarPathFinder(useMap:MapData)
		{
			//全图寻路
			this.xMapStart = 0;
			this.yMapStart = 0;
...
//			}
		...
		private function isWalkable(x:int, y:int):Boolean
		{
			...
			// 2. 是否是walkable
			return this.mapData.getWalkable(x, y);
		}
		...
		public function shortenPath(tilePointArr:Array, length:int):Array
		{
			...
			for (var i:int = tilePointArr.length-1; i > 0; i--)
			{
				...
				pixelPointTwo = MapGridUtil.getPixelPoint(mapData.tilePixelWidth, mapData.tilePixelHeight, mapData.mapPixelWidth, tilePointTwo.x, tilePointTwo.y);
				tilePointOne = tilePointArr[i-1];
				pixelPointOne = MapGridUtil.getPixelPoint(mapData.tilePixelWidth, mapData.tilePixelHeight, mapData.mapPixelWidth, tilePointOne.x, tilePointOne.y);
				...
				{
					resultArr.pop();
					tempTilePointArr = mapData.passedTile2(tilePointTwo, tilePointOne);
					for each (point in tempTilePointArr)
					{
						tempPixelPoint = MapGridUtil.getPixelPoint(mapData.tilePixelWidth, mapData.tilePixelHeight, mapData.mapPixelWidth, point.x, point.y);
						if (mapData.getWalkable(point.x, point.y) && Point.distance(pixelPointTwo, tempPixelPoint) >= lengthLeft)
						{
							resultArr.push(point);
							return resultArr;
						}
