package tx.sce.controller
{
	...
	public class LayerController extends Controller
	{
				...
				for each (var item:XML in xmlList)
				{
					if (item.parent().@name == "NPCLayer")
					{
						picArr.push({
							...
							x: item.@x,
							y: item.@y,
							mpOffsetX: item.@mpOffsetX,
							mpOffsetY: item.@mpOffsetY,
		...
		{
			outputMidOrFrontPics(data.layerData.layersArr["MiddleGround"], path,
					<SceneMiddlegroundConfig/>, width, height, "_scene_mg.xml");
			outputMidOrFrontPics(data.layerData.layersArr["FrontGround"], path,
					<SceneForegroundConfig/>, width, height, "_scene_fg.xml");
		}
		...
		private function editItemXml(xml:XML, id:int, bm:Bitmap, width:int, height:int, url:String):void
		{
			...
			for (var i:int = minX; i <= maxX; i++)
				for (var j:int = minY; j <= maxY; j++)
				{
					targetX = i * width;
					targetY = j * height;
					xml.appendChild(<img id={id} imgX={i} imgY={j} offsetX={bm.x - targetX} offsetY={bm.y - targetY}
										 url={url}/>);
				}
		}
		...
		private function showGrid():void
		{
			...
			msg.msgData = {
				...
				rows: data.layerData.gridRows,
				columns: data.layerData.gridColumns
			};
			dispatchMsg(msg);
		}
		...
		private function exportNPCLayer(path:String, imgWidth:int, imgHeight:int):void
		{
			...
			for (var i:int = 0; i < NPCLayer.length; i++)
			{
				...
				xml.appendChild(<NPC id={i} roleId={tmpXml.@roleId} res={"assets/role/npc/scene/npcres/" + render.name}
									 direction={render.idleIndex} destPoint={destPoint.x + dot + destPoint.y}
									 startImgX={startImgX} endImgX={endImgX} startImgY={startImgY} endImgY={endImgY}
									 canMove="false" movePath={movePath.x + dot + movePath.y}/>);
		...
		private function getLibraryXmlByName(name:String):XML
		{
			...
			var xml:XML = data.libraryData.libXml..item.(@name == str)[0];
			return xml;
		}
		...
		private function copyToPreView():void
		{
			...
			{
				GlobalData.tempDir.deleteDirectory(true);
			}
			GlobalData.tempDir = File.createTempDirectory();

			var destFile:File = new File(GlobalData.tempDir.resolvePath("ext/naruto/assets/scene/map/" + data.layerData.id).nativePath);
			file.copyTo(destFile, true);

			if (data.layerData.id != "1")
			{
				destFile.nativePath = GlobalData.tempDir.resolvePath("ext/naruto/assets/scene/map/1").nativePath;
				file.copyTo(destFile, true);
			}
			...
			if (file.exists)
			{
				destFile.nativePath = GlobalData.tempDir.resolvePath("ext/naruto/assets/role/npc/scene/npcres").nativePath;
				file.copyTo(destFile, true);
			}
			...
			destFile.nativePath = GlobalData.tempDir.resolvePath("ext/naruto/assets/role/npc/scene" + "/npccfg_scene1.xml").nativePath;
			file.copyTo(destFile, true);
		}
