package tx.sce.proxy
{
	...
	public class EditAreaProxy extends Proxy
	{
		...
		private var _movePicPosition:Point = new Point();   //移动场景背景图片时位置
		private const ZERO_POINT:Point = new Point();       //场景坐标原点
		private const PRECISE:int = 15;						//精度值，当场景图片小于此值就会吸附坐标原点
		private var _recordSelectPoint:Point = new Point();
		private var _statusLabel:Label;
		private var _recordMouseMovePoint:Point = new Point();//记录鼠标实时坐标
		...
		public function EditAreaProxy(winapp:MapEditor)
		{
			...
			app.leftBox.addChild(editAreaView);
			_statusLabel = app.statusLabel;
			initInteractItem();
		...
		private function onPicMouseDown(evt:Event):void
		{
			...
			_recordSelectPoint.setTo(selectedPoint.x, selectedPoint.y);
			_movePicPosition.setTo(pic.x, pic.y);
			updateStatusLable();
		}
		 ...
		 * 移动背景图片时
		 * */
		private function onUpdatePic(event:MouseEvent):void
		{
			if (selectedItem)
			{
				_movePicPosition.setTo(selectedItem.x, selectedItem.y);
				if (Point.distance(_movePicPosition, ZERO_POINT) <= PRECISE)
				{
					selectedItem.x = ZERO_POINT.x;
					selectedItem.y = ZERO_POINT.y;
				}
			}
		}
		...
		private function onMouseDown(evt:Event):void
		{
			...
			{
				if (editAreaView.view.mouseX >= 0 && editAreaView.view.mouseX <= editAreaView.canvas.width && editAreaView.view.mouseY >= 0 && editAreaView.view.mouseY <= editAreaView.canvas.height)
				{
					drawingGrid = true;
				}
			}
		...
		private function onMouseUp(evt:Event):void
		{
			...
			if (selectedItem)
			{
				if (!blueRect.visible)
				{
					...
					cmd.commandData = {
						layerName: selectedItem.parent.name,
						index: getLayerByName(selectedItem.parent.name).getChildIndex(selectedItem),
						x: selectedItem.x,
						y: selectedItem.y,
						oldX: selectedPoint.x,
						oldY: selectedPoint.y
					};
					dispatchCommand(cmd);
			...
			if (movePathViewDraging)
			{
				...
				cmd.commandData = {
					render: selectedItem,
					pathX: movePathView.x,
					pathY: movePathView.y,
					oldPathX: selectedPoint.x,
					oldPathY: selectedPoint.y
				};
				dispatchCommand(cmd);
				movePathViewDraging = false;
			}
		...
		private function changeGridState(canvasX:int, canvasY:int):void
		{
			...
			if (gridArr[gridPoint.x] && gridArr[gridPoint.x][gridPoint.y]) //判断有没有超界
			{
				...
				cmd.commandData = {
					x: gridPoint.x,
					y: gridPoint.y,
					walkable: !(gridArr[gridPoint.x][gridPoint.y] as Bitmap).visible
				};//如果网格砖块有显示则不能行走，所以取非
				dispatchCommand(cmd);
			}
		}
		...
		private function updatePicPos(layerName:String, index:int, x:int, y:int):void
		{
			...
			if (selectedItem == pic)
			{
				...
				else
				{
					...
					_recordSelectPoint.setTo(selectedItem.x, selectedItem.y);
					updateStatusLable();
				}
			}
		}

		private function updateStatusLable():void
		{
			if (_statusLabel)
			{
				_recordMouseMovePoint.setTo(canvas.mouseX, canvas.mouseY);
				var currentStr:String = "";
				var layerData:LayerData = GlobalData.layerData;
				var point:Point;
				if (selectedItem is NpcRender && movePathView)
				{
					if (movePathView.parent)
					{
						point = MapGridUtil.getTilePoint(layerData.gridWidth, layerData.gridHeight, movePathView.x + movePathView.parent.x, movePathView.y + movePathView.parent.y);
					} else
					{
						point = MapGridUtil.getTilePoint(layerData.gridWidth, layerData.gridHeight, movePathView.x + 15, movePathView.y + 15);
					}
					var selectName:String=selectedItem.name.split(".")[0];
					if(!selectName)
					{
						selectName="";
					}
					currentStr =selectName+ "：" + "行="+point.y+",列="+point.x;
				} else
				{
					point = MapGridUtil.getTilePoint(layerData.gridWidth, layerData.gridHeight, _recordMouseMovePoint.x,_recordMouseMovePoint.y);
					currentStr = "行="+point.y+",列="+point.x;
			}
				var totalStr:String = "";
				totalStr = "鼠标实时坐标：" + _recordMouseMovePoint + "    " + currentStr;
				_statusLabel.text = totalStr;
			}
		}
		...
		private function handlerRight():void
		{
			if (selectedItem is NpcRender)
			{
				var layerData:LayerData = GlobalData.layerData;
				var currentP:Point = MapGridUtil.getTilePoint(layerData.gridWidth, layerData.gridHeight, selectedItem.x, selectedItem.y);
				var targetP:Point = MapGridUtil.getPixelPoint(layerData.gridWidth, layerData.gridHeight, currentP.x, currentP.y);
				targetP.offset(32, -16);
				if (targetP.x < 0 || targetP.x > layerData.canvasWidth || targetP.y < 0 || targetP.y > layerData.canvasHeight)
				{
					return;
				}

				var cmd:Command = new Command(Command.MOVE_PIC);
				cmd.commandData = {
					layerName: selectedItem.parent.name,
					index: getLayerByName(selectedItem.parent.name).getChildIndex(selectedItem),
					x: targetP.x,
					y: targetP.y,
					oldX: _recordSelectPoint.x,
					oldY: _recordSelectPoint.y
				};
				dispatchCommand(cmd);
				(selectedItem.parent.name == "NPCLayer") && showMovePathView(selectedItem as NpcRender);
				//showRect要放在发送命令后面，因为这条命令会更改render的位置，而showRect是要根据render的位置来显示的。
		...
		private function handlerLeft():void
		{
			if (selectedItem is NpcRender)
			{
				var layerData:LayerData = GlobalData.layerData;
		...
		private function handlerDown():void
		{
			if (selectedItem is NpcRender)
			{
				var layerData:LayerData = GlobalData.layerData;
		...
		private function handlerUP():void
		{
			if (selectedItem is NpcRender)
			{
				var layerData:LayerData = GlobalData.layerData;
